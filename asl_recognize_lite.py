"""
ASL Fingerspelling Letter Recognition - TENSORFLOW LITE (EDGE OPTIMIZED)
- TFLite for 24 static letters (A-I, K-Y) - optimized for edge devices
...
class ASLLetterRecognizerLite:
    """Real-time ASL recognition with TFLite (edge optimized)"""
    
    def __init__(self, model_dir: str = "asl_data"):
        self.model_dir = Path(model_dir)
        
        # Load TFLite model
        self.interpreter = self._load_tflite_model("asl_static_model.tflite")
        ...
        self.min_trajectory_distance = 0.20
        ...
        self.stable_letter_threshold = 12
        
        print(f"✓ Loaded TFLite model (optimized for edge)")
        print(f"✓ Using rule-based motion detection for J/Z")
    
    def _load_tflite_model(self, filename: str):
        """Load TensorFlow Lite model"""
        filepath = self.model_dir / filename
        
        if not filepath.exists():
            print(f"⚠️ TFLite model not found: {filepath}")
            print("Run: python convertToTFlite.py")
            return None
    ...
    def recognize_frame(self, hand_landmarks):
        ...
        # Static letter recognition with TFLite
        if self.interpreter is not None and not self.motion_detection_active:
    ...
    def run_recognition(self, camera_id: int = 0):
        ...
        print("\n" + "="*70)
        print("ASL LETTER RECOGNITION - TFLITE (EDGE OPTIMIZED)")
        print("="*70)
        print("\nRecognizes all 26 letters!")
        print(" • 24 static letters: TFLite MLP (edge optimized)")
        ...
        with self.mp_hands.Hands(
            ...
            while True:
                ...
                # Draw UI
                cv2.putText(image, "TFLite Edge", (20, 30),
                ...
                cv2.imshow('ASL Letter Recognition (TFLite)', image)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ASL Letter Recognition - TFLite Edge Optimized'
    )
    parser.add_argument(
        '--model-dir',
        type=str,
        default='asl_data',
        help='Directory containing TFLite model'
    )
    parser.add_argument(
        '--camera',
        type=int,
    ...
    recognizer = ASLLetterRecognizerLite(args.model_dir)
