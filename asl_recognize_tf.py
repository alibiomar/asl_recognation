"""
ASL Fingerspelling Letter Recognition - TENSORFLOW + RULE-BASED MOTION
...
class MotionTrajectory:
    ...
    def clear(self):
        """Clear all points from the trajectory"""
        self.points.clear()
...
class ASLLetterRecognizer:
    ...
    def extract_static_features(self, hand_landmarks) -> Optional[np.ndarray]:
        ...
        if self.static_metadata and 'normalization_params' in self.static_metadata:
            mean = self.static_metadata['normalization_params']['mean']
            std = self.static_metadata['normalization_params']['std']
    ...
    def detect_motion_start(self, hand_landmarks, prev_landmarks) -> bool:
        ...
        # Trigger motion mode after consecutive movements
    ...
    def classify_motion_trajectory(self) -> Tuple[Optional[str], float]:
        ...
        # Rule 1: Finger type (most important)
        ...
        # Rule 2: Path length (should be deliberate)
        ...
        with self.mp_hands.Hands(
            ...
            while True:
                ...
                if key == ord('q'):
                    ...
                    status = "enabled" if self.tts.toggle() else "disabled"
                    print(f"TTS {status}")
