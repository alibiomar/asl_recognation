"""
ASL Fingerspelling Letter Recognition - FIXED VERSION
- TensorFlow/Keras MLP for 24 static letters (A-I, K-Y)
...
class ASLLetterRecognizer:
    """Real-time ASL recognition with TensorFlow MLP + Rule-based Motion"""
    ...
    def __init__(self, model_dir: str = "asl_data", voice_path: str = None):
        ...
        self.static_model = self._load_tensorflow_model("asl_static_model.h5")
        self.static_metadata = self._load_metadata("asl_static_metadata.pkl")
        
        # Extract and convert normalization parameters
        self.mean = None
        self.std = None
        if self.static_metadata and 'normalization_params' in self.static_metadata:
            ...
            print(f"✓ Loaded normalization params (mean shape: {self.mean.shape})")
        ...
        print(f"✓ Loaded static model (MLP for 24 letters)")
        print(f"✓ Using rule-based motion detection for J/Z")
    
    def _load_tensorflow_model(self, filename: str):
    ...
    def _load_metadata(self, filename: str):
        ...
        if not filepath.exists():
            print(f"❌ Metadata not found: {filepath}")
            return None
    ...
    def extract_static_features(self, hand_landmarks) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.mean is not None and self.std is not None:
                landmarks = (landmarks - self.mean) / (self.std + 1e-8)
    ...
    def classify_motion_trajectory(self) -> Tuple[Optional[str], float]:
        ...
        J = pinky finger drawing downward motion
        Z = index finger drawing zigzag pattern
        """
        features = self.motion_trajectory.get_features()
        finger_type = self.motion_trajectory.finger_type
        
        # Must have identified finger
        if finger_type not in ["pinky", "index"]:
            return None, 0.0
        
        # Must have sufficient movement
        ...
        # Rule 3: Motion characteristics
        if letter == 'Z':
            # Z has more direction changes (zigzag)
            if features['direction_changes'] >= 2:
                confidence += 0.1
            if features['straightness'] < 0.6:
                confidence += 0.05
        elif letter == 'J':
            # J is smoother with vertical movement
    ...
    def recognize_frame(self, hand_landmarks):
        ...
        if self.static_model is not None and not self.motion_detection_active:
            static_features = self.extract_static_features(hand_landmarks)
            
            if static_features is not None:
                try:
                    # Predict with TensorFlow
                    features = np.expand_dims(static_features, axis=0)
                    prediction = self.static_model.predict(features, verbose=0)[0]
    ...
    def run_recognition(self, camera_id: int = 0):
        ...
        print("\n" + "="*70)
        print("ASL LETTER RECOGNITION - MLP + RULE-BASED MOTION")
        print("="*70)
        print("\nRecognizes all 26 letters!")
        print(" • 24 static letters: MLP neural network")
        ...
        with self.mp_hands.Hands(
            ...
            while True:
                ...
                if results.multi_hand_landmarks:
                    ...
                    if self.motion_detection_active and not self.motion_trajectory.is_empty():
                        ...
                        for i in range(1, len(points)):
                            x1 = int(points[i-1][0] * w)
                            y1 = int(points[i-1][1] * h)
                            x2 = int(points[i][0] * w)
                            y2 = int(points[i][1] * h)
                            cv2.line(image, (x1, y1), (x2, y2), (0, 255, 0), 3)
                        
                        # Draw finger tip trajectory
                        ...
                        if finger_points:
                            ...
                            for i in range(1, len(finger_points)):
                                x1 = int(finger_points[i-1][0] * w)
                                y1 = int(finger_points[i-1][1] * h)
                                x2 = int(finger_points[i][0] * w)
                                y2 = int(finger_points[i][1] * h)
                ...
                # Draw UI
                cv2.putText(image, "MLP + Rules", (20, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (100, 200, 255), 2)
                
                # Prediction display
                if current_letter:
                    pred_box_h = 150
                    cv2.rectangle(image, (w//2 - 100, 20), (w//2 + 100, 20 + pred_box_h),
                                (0, 0, 0), -1)
                    cv2.rectangle(image, (w//2 - 100, 20), (w//2 + 100, 20 + pred_box_h),
                                (0, 255, 0), 3)
                ...
                if key == ord('q'):
                    ...
                    if self.detected_word:
                        ...
                        # After speaking, insert a single space so the next
                        # letters you add start a new word visually.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ASL Letter Recognition - MLP + Rule-based Motion',
...
Examples:
  ...
  python asl_recognize.py --voice path/to/voice.onnx # Enable TTS
    ...
    parser.add_argument(
        ...
        type=str,
        default='asl_data',
        help='Directory containing trained models (default: asl_data)'
    )
    parser.add_argument(
        '--camera',
        type=int,
